...
@testset "pbs_size_gb" begin
    ...
    @test Physics.pbs_size_gb("58720256kb") == 56.0
    @test Physics.pbs_size_gb("126gb") == 126.0
    @test Physics.pbs_size_gb("2tb") == 2048.0
    @test Physics.pbs_size_gb("garbage") == 0.0
end

@testset "parse_pbsnodes" begin
    vnodes = Physics.parse_pbsnodes(PBSNODES)
...
@testset "cluster_capacities" begin
    ...
    caps = Physics.cluster_capacities(PROBE, ["defaultQ"], 1, 1.0)
    @test caps == ["defaultQ" => 15] # nodes fit 6+9=15 < 40 allowance
    @test Physics.cluster_capacities("", ["defaultQ"], 1, 1.0) == ["defaultQ" => 0]
end
...
@testset "parse_hpc_capacity" begin
    ...
    @test Physics.parse_hpc_capacity(out, 1, 4.0, 0.75) == 8
    @test Physics.parse_hpc_capacity(out, 4, 4.0, 0.75) == 2
    @test Physics.parse_hpc_capacity("", 1, 4.0, 0.75) == 0
    @test Physics.parse_hpc_capacity("8\n20.0 19.0 18.0 1/1 1\nMem: 62 42 0 0 19 19\n",
                                     1, 4.0, 0.75) == 0 # overloaded machine
    # existing workers reduce capacity: 4 reserved cores -> 8.9 - 4 = 4.9 workers
    @test Physics.parse_hpc_capacity(out, 1, 4.0, 0.75; reserved_cores = 4) == 4
    # reserved memory binds: 85*0.75 - 48 = 15.75 -> 3 workers of 4GB
    @test Physics.parse_hpc_capacity(out, 1, 4.0, 0.75; reserved_gb = 48.0) == 3
    @test Physics.parse_hpc_capacity(out, 1, 4.0, 0.75; reserved_cores = 100) == 0
end

@testset "reserved_on" begin
    entry(pid) = (pid = pid, cpus = 2, gb = 4.0)
    empty!(Physics.HPC_WORKERS)
    @test Physics.reserved_on("orr") == (0, 0.0)
    # pid 1 (this process) is always live; pid 9999 is dead and gets pruned
    Physics.HPC_WORKERS["orr"] = [entry(1), entry(9999)]
    @test Physics.reserved_on("orr") == (2, 4.0)
    @test length(Physics.HPC_WORKERS["orr"]) == 1 # dead entry pruned in place
    empty!(Physics.HPC_WORKERS)
end
...
@testset "allocate_workers" begin
    ...
    # extremes
    calloc, halloc, short = Physics.allocate_workers(10, cluster, hpc, 0)
    @test sum(last, calloc) == 10 && sum(last, halloc) == 0 && short == 0
    calloc, halloc, short = Physics.allocate_workers(20, cluster, hpc, 0)
    @test sum(last, calloc) == 15 && sum(last, halloc) == 0 && short == 5 # no spill at 0
    calloc, halloc, short = Physics.allocate_workers(10, cluster, hpc, 1)
    @test sum(last, calloc) == 0 && sum(last, halloc) == 10 && short == 0
    # balanced: capacities 15 vs 15 -> even split
    calloc, halloc, short = Physics.allocate_workers(10, cluster, hpc, 0.5)
    ...
    # bias toward hpc
    _, halloc, _ = Physics.allocate_workers(10, cluster, hpc, 0.9)
    @test sum(last, halloc) > 5
    # spill: hpc share exceeds hpc capacity -> cluster absorbs
    calloc, halloc, short = Physics.allocate_workers(30, cluster, hpc, 0.5)
    @test sum(last, calloc) == 15 && sum(last, halloc) == 15 && short == 0
    calloc, halloc, short = Physics.allocate_workers(40, cluster, hpc, 0.5)
    @test short == 10
...
@testset "proportional_split" begin
    ...
    # respects caps
    split7 = Physics.proportional_split(7, ["a" => 1, "b" => 6])
    @test split7 == ["a" => 1, "b" => 6]
end
